    ...
    <div>
      ...
      <div className="mt-6 grid gap-6 xl:grid-cols-[1.5fr_.8fr]">
        <section className="surface-card overflow-hidden">
          <div className="flex items-center justify-between border-b border-slate-100 p-5"><div><h2 className="font-semibold">Son aramalar</h2><p className="mt-1 text-xs text-slate-400">En son oluşturduğunuz hedef listeleri</p></div><Link href="/panel/gecmis" className="text-sm font-semibold text-blue-800">Tümünü gör</Link></div>
        ...
        <section className="rounded-2xl bg-blue-950 p-6 text-white shadow-xl shadow-blue-950/10"><span className="grid h-11 w-11 place-items-center rounded-xl bg-white/10"><Sparkles className="h-5 w-5 text-blue-200"/></span><h2 className="mt-12 text-2xl font-semibold tracking-tight">İlk hedef listenizi oluşturun.</h2><p className="mt-3 text-sm leading-6 text-blue-100/70">Sektör ve bölgeyi tanımlayın. SalesPilot gerçek alıcıları bulup kanıtlarıyla birlikte puanlasın.</p><Link href="/panel/yeni-arama" className="mt-7 inline-flex items-center gap-2 rounded-lg bg-white px-5 py-3 text-sm font-semibold text-blue-950">Aramaya başla <ArrowRight className="h-4 w-4"/></Link></section>
      </div>
    </div>
  );
}
