  ...
  return (
    <main className="grid min-h-screen bg-[#f5f7f3] lg:grid-cols-2">
      <section className="hidden bg-emerald-950 p-12 text-white lg:flex lg:flex-col">
        <Link href="/" className="flex items-center gap-3 font-semibold"><span className="grid h-10 w-10 place-items-center rounded-xl bg-white text-xs text-emerald-950">SP</span>SalesPilot</Link>
        <div className="my-auto max-w-lg"><p className="text-xs font-semibold uppercase tracking-[.2em] text-emerald-300">Satış çalışma alanı</p><h1 className="mt-5 text-5xl font-semibold leading-[1.05] tracking-[-.05em]">Doğru müşteriye giden daha akıllı yol.</h1><p className="mt-6 text-lg leading-8 text-emerald-100/65">Hedef şirketleri keşfedin, gerçek alıcıları doğrulayın ve her kararı kanıtla destekleyin.</p><div className="mt-10 space-y-4">{["Kanıta dayalı lead puanlama", "Rakip ve satıcıları otomatik eleme", "Kontrollü e-posta iş akışı"].map((item) => <p key={item} className="flex items-center gap-3 text-sm text-emerald-50"><CheckCircle2 className="h-5 w-5 text-emerald-300"/>{item}</p>)}</div></div>
        <p className="text-xs text-emerald-100/40">SalesPilot · Pilot sürüm</p>
      </section>
      ...
      <span className="grid h-12 w-12 place-items-center rounded-2xl bg-emerald-100 text-emerald-800"><LockKeyhole className="h-5 w-5"/></span>
