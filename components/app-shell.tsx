...
export function AppShell({ children }: { children: React.ReactNode }) {
  ...
  const nav = (items: typeof navigation) => items.map(({ href, label, icon: Icon }) => {
    ...
    return (
      <Link key={href} href={href} onClick={() => setMobileOpen(false)} className={`flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm font-medium transition ${active ? "bg-emerald-50 text-emerald-900" : "text-slate-500 hover:bg-slate-50 hover:text-slate-900"}`}>
  ...
  const sidebar = (
    <div className="flex h-full flex-col bg-white">
      <div className="flex h-20 items-center justify-between px-5">
        <Link href="/panel" className="flex items-center gap-3 font-semibold tracking-tight"><span className="grid h-9 w-9 place-items-center rounded-xl bg-emerald-950 text-xs text-white">SP</span>SalesPilot</Link>
        <button className="lg:hidden" onClick={() => setMobileOpen(false)} aria-label="Menüyü kapat"><X className="h-5 w-5" /></button>
      </div>
      <div className="px-3"><Link href="/panel/yeni-arama" className="flex items-center justify-center gap-2 rounded-xl bg-emerald-950 px-4 py-3 text-sm font-semibold text-white shadow-lg shadow-emerald-950/10 transition hover:-translate-y-0.5"><Sparkles className="h-4 w-4" />Yeni tarama başlat</Link></div>
      ...
      <div className="mt-auto border-t border-slate-100 p-3">
        ...
        <Link href="/panel/profil" className="mt-3 flex items-center gap-3 rounded-xl border border-slate-200 p-3 hover:bg-slate-50">
          <span className="grid h-9 w-9 place-items-center rounded-full bg-emerald-100 text-xs font-semibold text-emerald-800">{profileName.split(" ").slice(0, 2).map((part) => part[0]).join("").toUpperCase()}</span>
  ...
  return (
    <div className="min-h-screen bg-[#f5f7f6] text-slate-950">
      ...
      <div className="lg:pl-64">
        <header className="sticky top-0 z-20 flex h-16 items-center justify-between border-b border-slate-200/80 bg-[#f5f7f6]/90 px-5 backdrop-blur lg:px-8">
          ...
          <div className="flex items-center gap-2 text-xs text-slate-500"><span className="h-2 w-2 rounded-full bg-emerald-500"/>Sistem hazır</div>
        </header>
